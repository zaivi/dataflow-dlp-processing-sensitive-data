...
public class DLPTextToBigQueryStreaming {
  ...
  public static void main(String[] args) {
    String[] pipelineOptions = {
        ...
        "--dlpProjectId=ext-pinetree-dw",
        "--region=asia-southeast1",
        "--subnetwork=https://www.googleapis.com/compute/v1/projects/ext-pinetree-dw/regions/asia-southeast1/subnetworks/sb-dw-sing",
        ...
        "--batchSize=500",
        "--deidentifyTemplateName=projects/ext-pinetree-dw/locations/global/deidentifyTemplates/408350857443695353",
        // "--tempLocation=gs://dataflow-staging-asia-southeast1-371406354181/tmp",
        // "--inputFilePattern=*.csv.gz",
        "--inputFilePattern=BIZ-TAC_IO_REQUEST-0.csv.gz",
        "--dlpBucket=ext-pinetree-dw-dataflow-migration-dlp",
        "--dlpFolder=oracle-mini-batching",
        "--bigQueryLoadingTemporaryDirectory=gs://ext-pinetree-dw-test-dlp/temp",
        "--experiments=enable_streaming_engine",
        "--datasetName=dev_dl_oracle_input_raw",
    ...
    GCSUtils GCSObject = new GCSUtils();
    GCSObject.DeleteObjectsBeforeDLP(options.getDlpProjectId().get(),
        options.getDlpBucket().get(), options.getDlpFolder().get());
  ...
  public static PipelineResult run(TokenizePipelineOptions options) {
    ...
    String ObjectsPattern = String.format("gs://%s/%s/%s", options.getDlpBucket().get(), options.getDlpFolder().get(),
        options.getInputFilePattern().get());
    String projectId = options.getDlpProjectId().get().toString();
    String datasetName = options.getDatasetName().get().toString();
    ...
    bqDataMap.apply(
        ...
        BigQueryIO.<KV<String, TableRow>>write()
            .to(input -> getDestination(input, projectId, datasetName))
            .withFormatFunction(
                element -> {
  ...
  public interface TokenizePipelineOptions extends DataflowPipelineOptions {
    ...
    @TemplateParameter.Text(order = 5, regexes = {
        "^[^.]*$" }, description = "BigQuery Dataset", helpText = "BigQuery Dataset to be used. Dataset must exist prior to execution. Ex. pii_dataset")
    ValueProvider<String> getDatasetName();

    void setDatasetName(ValueProvider<String> value);

    @TemplateParameter.ProjectId(order = 6, description = "Cloud DLP project ID", helpText = "Cloud DLP project ID to be used for data masking/tokenization. Ex. your-dlp-project")
    ValueProvider<String> getDlpProjectId();

    void setDlpProjectId(ValueProvider<String> value);

    @TemplateParameter.Text(order = 5, regexes = {
        "^[^.]*$" }, description = "GCS DLP Bucket", helpText = "GCS bucket contain dlp data for source. Ex. ext-pinetree-dw-dataflow-migration-dlp")
    ValueProvider<String> getDlpBucket();

    void setDlpBucket(ValueProvider<String> value);

    @TemplateParameter.Text(order = 5, regexes = {
        "^[^.]*$" }, description = "GCS DLP Folder", helpText = "GCS folder of bucket contain dlp data for source. Ex. oracle-batching")
    ValueProvider<String> getDlpFolder();

    void setDlpFolder(ValueProvider<String> value);
    ...
    @TemplateParameter.Text(order = 5, regexes = {
        "^[^.]*$" }, description = "File Delimiter", helpText = "Choose a delimiter of file load. Ex. ',' for comma delimiter")
  ...
  static class CSVReader extends DoFn<KV<String, ReadableFile>, KV<String, Table>> {
    ...
    public CSVReader(ValueProvider<Integer> batchSize, ValueProvider<String> delimiter) {
      lineCount = 1;
    ...
    public void processElement(ProcessContext c, RestrictionTracker<OffsetRange, Long> tracker)
      ...
      for (long i = tracker.currentRestriction().getFrom(); tracker.tryClaim(i); ++i) {
        ...
        try (BufferedReader br = getReader(c.element().getValue())) {
          ...
          Iterator<CSVRecord> csvRows = CSVFormat.newFormat(delimiter.get().charAt(0)).parse(br).iterator();
          if (!csvRows.hasNext()) {
  ...
  static class DLPTokenizationDoFn extends DoFn<KV<String, Table>, KV<String, Table>> {
    ...
    public void processElement(ProcessContext c) {
      ...
      try {
        ...
        LOG.error("Failed {}", e.getMessage().toString());
        LOG.error("Failed to process file {}", key);
      }
    }
  }
  ...
  private static String getFileName(ReadableFile file) {
    ...
    String[] tempTableName = fileKey[0].split("\\-");
    String tableName = tempTableName[0] + "-" + tempTableName[1];
  ...
  private static BufferedReader getReader(ReadableFile csvFile) {
    ...
    try {
      ...
      if (channel != null) {
        ...
        br = new BufferedReader(new InputStreamReader(Channels.newInputStream(channel), "UTF8"));
      }
      return br;
    } catch (IOException e) {
  ...
  static TableDestination getDestination(ValueInSingleWindow<KV<String, TableRow>> element,
      String outputProject,
      String outputDataset) {
    String key = element.getValue().getKey();
    String tableName = String.format("%s:%s.%s", outputProject, outputDataset, key);
